...
plot(boost.model, i = "retention_rate")
plot(boost.model, i = "sat_median")

results <- data.frame(
  Model = c("Linear", "Elastic Net", "Random Forest", "Bagging", "Boosting"),
  Test_MSE = c(mse.lm, mse.enet, mse.rf, mse.bag, mse.boost),
  Test_RMSE = c(rmse.lm, rmse.enet, rmse.rf, rmse.bag, rmse.boost),
  Test_R2 = c(r2.lm, r2.enet, r2.rf, r2.bag, r2.boost)
)
